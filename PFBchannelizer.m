function PFBchannelizer(filename_in, nseries, os_factor, method_, verbose_, diagnostic_plots_)
  %
  ...
  % Output file name
  filepath_split = strsplit(filename_in, '/');
  filename_in_split = strsplit(filepath_split{end}, '.');
  input_signal_type = filename_in_split{1};

  method
  if method == 1
    prefix = sprintf('full_channelized_%s', input_signal_type);
  elseif method == 2
    prefix = sprintf('full_channelized_%s.polyphase_analysis', input_signal_type);
  end

  fname_out = strrep(filename_in, input_signal_type, prefix);
  ...
  fir_filter_coef = single(FiltCoefStruct.h);
  ...
  for ii=1:nseries
      ...
      for p = 1:npol
        ...
        elseif method == 2
          res = polyphase_analysis(Vdat(p,:), fir_filter_coef, L, chunk_size);
          y2(p,:,:) = transpose(res);
        end
      end
...
function PFB = PFB_factory(Nchan, pfb_filter_coef_fname, OS_Nu, OS_De, output_ndim_, ntype_, verbose_)
  ...
  function y = OS_PFB(x)
    ...
    if n == 0
        ...
        idx = [(OS_Nu-n)*L_M+1:L 1:(OS_Nu-n)*L_M];
