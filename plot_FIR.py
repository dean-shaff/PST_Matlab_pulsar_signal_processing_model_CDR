...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Plot the Finite Impulse Response stored in a MATLAB .mat file")
...
def main():
    ...
    axes[1].plot(pass_band_x, -0.5*Ap*np.ones(n_points))
    axes[1].plot(pass_band_x, 0.5*Ap*np.ones(n_points))
    ...
    axes[2].plot(stop_band_x, -As*np.ones(n_points))
    axes[2].set_xlim([0, 1])
