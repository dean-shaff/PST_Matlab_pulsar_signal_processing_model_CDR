function [H, f, n_hi, n_lo] = ...
                dispnmatrix(frange,usable_BW,Nout,nfreq,DD,Tout,direc)
...
% discarded.
if abs(usable_BW) < abs(bandwidth),
    disp('Discarding regions outside the usable bandwidth');
    discard = (abs(bandwidth) - abs(usable_BW))/abs(bandwidth)/2.0;
    H(1:floor(discard*Nout)) = complex(0,0);
    H(ceil((1.0-discard)*Nout):Nout) = complex(0,0);
end;
